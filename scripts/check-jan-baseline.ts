...
async function checkBaseline() {
  ...
  console.log(`🔍 Checking baseline for ${date}...`);

  // 1. Check inventory table for snapshots around that date
  const snapshotCheck = await executeSQL(`
    SELECT DISTINCT DATE(imported_at) as import_date, COUNT(*) as row_count
    FROM inventory
    GROUP BY import_date
    ORDER BY import_date DESC
  `);
  console.log('Available snapshots in inventory table:', snapshotCheck.rows);

  // 2. Query for Auto Flagship baseline on Jan 31st
  // Auto: 품목그룹1코드 IN ('PVL', 'CVL')
  // Flagship: 품목그룹3코드 = 'FLA'
  const query = `
    SELECT 
  ...
  console.log('\n📊 Auto Flagship Baseline on Jan 31st:', result.rows[0]?.total_qty);
}
