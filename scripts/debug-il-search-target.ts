...
async function main() {
  ...
  const combinations = [
    { name: 'Meeting (Filtered Sales + PR00061)', wh: false, salesFiltered: true, purFiltered: true },
    { name: 'Meeting + WH', wh: true, salesFiltered: true, purFiltered: true },
    { name: 'Filtered Sales + All Purchases', wh: false, salesFiltered: true, purFiltered: false },
    { name: 'Raw (No Filters)', wh: false, salesFiltered: false, purFiltered: false },
  ];
  ...
  for (const c of combinations) {
    ...
    for (const m of ['2026-01', '2026-02', '2026-03']) {
      ...
      inv = inv + p - s;
      if (m === '2026-03') {
        console.log(`${c.name}: ${Math.round(inv).toLocaleString('ko-KR')}`);
      }
    }
  }
}
