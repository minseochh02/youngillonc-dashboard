...
async function fetchFebruaryMobilWeightStepByStep() {
  try {
    ...
    for (const table of tables) {
      ...
      const q = `
        ...
        LEFT JOIN employees e ON s.${table.joinCol} = ${table.joinTo}
        WHERE substr(s.일자, 1, 7) = '${targetMonth}'
          AND (e.사원_담당_명 IS NULL OR e.사원_담당_명 != '${excludedEmployee}')
          AND ${filter}
      `;
...
fetchFebruaryMobilWeightStepByStep();
