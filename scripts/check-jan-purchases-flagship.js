...
async function main() {
  console.log(`🔍 Checking January Purchase Liters for Auto Flagship...`);

  const query = `
    SELECT 
      SUM(CAST(REPLACE(p.중량, ',', '') AS NUMERIC)) as total_liters
    FROM purchases p
  ...
  console.log('\n📊 Total Auto Flagship Liters Purchased in January:', result.rows[0]?.total_liters);
}
