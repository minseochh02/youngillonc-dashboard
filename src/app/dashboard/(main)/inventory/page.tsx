...
export default function InventoryStatusPage() {
  ...
  const [dailyViewMode, setDailyViewMode] = useState<'division' | 'office'>('division');
  ...
  const fetchDailyData = async () => {
    ...
    setError(null);
    try {
      const response = await apiFetch(`/api/dashboard/daily-status/inventory-sheet?date=${date}&viewMode=${dailyViewMode}`);
      const result = await response.json();
      if (result.success) {
  ...
  const handleExcelDownload = async () => {
    ...
    try {
      ...
      const [itemsRes, dailyRes] = await Promise.all([
        ...
        apiFetch(`/api/dashboard/daily-status/inventory-sheet?date=${date}&viewMode=${dailyViewMode}`).then(r => r.json())
      ]);
    ...
    <div className="space-y-6">
              ...
              <table className="w-full text-sm">
                <thead className="sticky top-0 z-10 bg-zinc-50 dark:bg-zinc-800/80 backdrop-blur">
                  <tr>
                    ...
                    <th className="text-center py-3 px-3 text-[10px] font-bold text-zinc-500 uppercase tracking-wider border-b border-zinc-200 dark:border-zinc-700 min-w-[100px]">
                      상태
                    </th>
                  </tr>
                </thead>
      ...
      {activeTab === 'daily' && (
        <>
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center bg-zinc-100 dark:bg-zinc-800 p-1 rounded-lg border border-zinc-200 dark:border-zinc-700">
                <button
                  onClick={() => setDailyViewMode('division')}
                  className={`px-3 py-1.5 text-xs font-bold rounded-md transition-all ${
                    dailyViewMode === 'division'
                ...
                <button
                  onClick={() => setDailyViewMode('office')}
                  className={`px-3 py-1.5 text-xs font-bold rounded-md transition-all ${
                    dailyViewMode === 'office'
                      ? 'bg-blue-600 text-white shadow-sm'
                      : 'text-zinc-600 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-200'
                  }`}
                >
          ...
          {/* Branch Selection Filter (only visible in office detailed view mode) */}
          {dailyData && dailyViewMode === 'office' && (
              ...
              <table className="w-full text-sm border-collapse table-fixed">
                <thead>
                  <tr className="bg-zinc-50 dark:bg-zinc-900/80 backdrop-blur-md">
                    <th className="sticky left-0 z-20 bg-zinc-50 dark:bg-zinc-900 py-2.5 px-3 border-b border-r border-zinc-200 dark:border-zinc-800 text-left w-[100px]">분류</th>
                    <th className="sticky left-[100px] z-20 bg-zinc-50 dark:bg-zinc-900 py-2.5 px-3 border-b border-r border-zinc-200 dark:border-zinc-800 text-left w-[70px]">산업군</th>
                    <th className="sticky left-[170px] z-20 bg-zinc-50 dark:bg-zinc-900 py-2.5 px-3 border-b border-r border-zinc-200 dark:border-zinc-800 text-left w-[75px]">티어</th>
                    {branchesToShow.map(col => (
                      <th 
                        key={col} 
                        className={`py-2.5 px-3 border-b border-zinc-200 dark:border-zinc-800 text-center w-[140px] min-w-[140px] ${
                          col === '합계' 
                  ...
                  {METRICS.map((metric) => (
                    <React.Fragment key={metric.id}>
                        ...
                        >
                          {catIdx === 0 && (
                            <td
                              ...
                              className="sticky left-0 z-10 bg-white dark:bg-zinc-950 px-3 py-2.5 border-r border-zinc-200 dark:border-zinc-800 font-bold text-zinc-900 dark:text-zinc-100 align-middle w-[100px]"
                            >
                          ...
                          <td className="sticky left-[100px] z-10 bg-white dark:bg-zinc-950 px-3 py-2.5 border-r border-zinc-200 dark:border-zinc-800 font-medium text-zinc-700 dark:text-zinc-300 w-[70px]">
                            {cat.label}
                          </td>
                          <td className="sticky left-[170px] z-10 bg-white dark:bg-zinc-950 px-3 py-2.5 border-r border-zinc-200 dark:border-zinc-800 text-xs text-zinc-500 dark:text-zinc-500 w-[75px]">
                            {cat.subLabel}
                          </td>
                          {branchesToShow.map(col => {
                            let qty = 0;
                            ...
                            if (dailyData?.stats) {
                              Object.entries(dailyData.stats).forEach(([branchName, bStats]) => {
                            ...
                            return (
                              <td 
                                key={col} 
                                className={`py-2.5 px-3 border-r border-zinc-100 dark:border-zinc-900/50 align-middle ${
                                  col === '합계' 
                              ...
                              >
                                <div className="flex flex-col items-end gap-1">
                                  ...
                                  } ${col === '합계' ? 'text-blue-600 dark:text-blue-400' : ''}`}>
                                    {fmt(qty)}
                      ...
                      <tr className="bg-zinc-100/90 dark:bg-zinc-900/90 font-bold border-b-2 border-zinc-300 dark:border-zinc-700">
                        <td colSpan={2} className="sticky left-0 z-10 bg-zinc-100 dark:bg-zinc-900 px-3 py-2 border-r border-zinc-300 dark:border-zinc-700 text-xs font-bold text-zinc-900 dark:text-zinc-100 w-[170px]">
                          {metric.label} 합계 (소계)
                        </td>
                        {branchesToShow.map(col => {
                          let totQty = 0;
                          ...
                          return (
                            <td
                              key={col}
                              className={`py-2 px-3 border-r border-zinc-200 dark:border-zinc-800 align-middle ${
                                col === '합계'
                ...
                {dailyData && (() => {
                  const getMetricSummaryStats = (metricId: 'purchase' | 'sales' | 'inventory', col: string) => {
                    let weight = 0;
                    let qty = 0;
                    if (!dailyData?.stats) return { weight, qty, pct: 0 };
                  ...
                  return (
                    <tfoot className="bg-zinc-100/80 dark:bg-zinc-900/80 border-t-4 border-zinc-300 dark:border-zinc-700 font-bold">
                      ...
                      {[
                        ...
                        if (dailyData?.stats) {
                          Object.entries(dailyData.stats).forEach(([bName, bStats]) => {
                        ...
                        return (
                          <tr key={`div-row-${div.name}`} className="bg-indigo-50/70 dark:bg-indigo-950/40 border-b border-indigo-100 dark:border-indigo-900/40 text-xs">
                            <td className="sticky left-0 z-20 bg-indigo-50 dark:bg-indigo-950 py-1.5 px-2 border-r border-indigo-200 dark:border-indigo-800 font-bold text-indigo-950 dark:text-indigo-100 w-[100px]">
                              └ {div.name}
                            </td>
                            <td className="sticky left-[100px] z-10 bg-indigo-50 dark:bg-indigo-950 py-1.5 px-2 border-r border-indigo-200 dark:border-indigo-800 font-medium text-indigo-700 dark:text-indigo-300 w-[70px]">
                              {div.label}
                            </td>
                            <td className="sticky left-[170px] z-10 bg-indigo-50 dark:bg-indigo-950 py-1.5 px-2 border-r border-indigo-200 dark:border-indigo-800 text-xs text-indigo-600 dark:text-indigo-400 font-bold w-[75px]">
                              ({pct.toFixed(1)}%)
                            </td>
                            {branchesToShow.map(col => {
                              let weight = 0;
                              ...
                              return (
                                <td
                                  key={col}
                                  className={`py-1.5 px-2 border-r border-indigo-100 dark:border-indigo-900/40 align-middle ${
                                    col === '합계'
