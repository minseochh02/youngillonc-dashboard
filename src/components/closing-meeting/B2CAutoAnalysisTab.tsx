...
export default function B2CAutoAnalysisTab({ selectedMonth }: B2CAutoAnalysisProps) {
  ...
  const expandAll = () => {
    const allBranches = new Set(data?.branches.map((b: BranchAutoData) => b.branch));
    setExpandedBranches(allBranches);
  };
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl overflow-hidden">
        <div className="px-6 py-4 border-b border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-800/50 flex items-center justify-between">
          <h4 className="text-sm font-bold text-zinc-900 dark:text-zinc-100">지사별 B2C AUTO 분석</h4>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {data.branches.map((branch) => {
                ...
                const momChange = branch.current_month_weight - branch.last_month_weight;
                const momChangeRate = branch.last_month_weight > 0
      ...
      <div className="text-xs text-zinc-500 dark:text-zinc-400 bg-zinc-50 dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-800 rounded-lg p-4">
        <p className="font-semibold mb-1">B2C AUTO 분석:</p>
        <p>B2C 사업부의 자동차(AUTO) 업종 판매 실적을 지사별, 팀별로 분석한 데이터입니다. 지사를 클릭하면 팀별 상세 데이터를 확인할 수 있습니다.</p>
      </div>
    </div>
  );
}
