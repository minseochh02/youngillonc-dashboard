...
export default function TeamVolumeTab({ selectedMonth }: TeamVolumeTabProps) {
  ...
  const { currentYear, lastYear, volumeData } = data;
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="bg-gradient-to-r from-blue-50 to-cyan-50 dark:from-blue-950/20 dark:to-cyan-950/20 border border-blue-200 dark:border-blue-800 rounded-xl p-6">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h3 className="text-lg font-bold text-zinc-900 dark:text-zinc-100">최근 월별 물량 (1월~{parseInt(currentMonthNum)}월)</h3>
      ...
      <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl overflow-hidden">
        <div className="px-6 py-4 border-b border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-800/50">
          <h4 className="text-sm font-bold text-zinc-900 dark:text-zinc-100">
            {currentYear}년 팀별 담당자별 물량 (AUTO) - 1월~{parseInt(currentMonthNum)}월
          </h4>
        </div>
            ...
            <tbody>
              {Array.from(tableData.entries()).map(([team, teamMap]) => {
                return Array.from(teamMap.entries()).map(([employeeName, employeeMap]) => {
                  return Array.from(employeeMap.entries()).map(([productGroup, productMap]) => {
