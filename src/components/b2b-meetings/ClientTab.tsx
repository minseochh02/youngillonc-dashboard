...
export default function ClientTab() {
  ...
  const filteredData = customerData.filter(row => {
    ...
    return (
      ...
      row.담당자?.toLowerCase().includes(search) ||
      row.사업소?.toLowerCase().includes(search) ||
      row.B2B팀?.toLowerCase().includes(search) ||
      row.섹터분류?.toLowerCase().includes(search)
    );
  });
  ...
  const handleExcelDownload = () => {
    ...
    const filename = generateFilename('B2B_거래처별_판매현황');
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center">
        <div className="relative flex-1 max-w-md">
          ...
          <input
            type="text"
            placeholder="거래처명, 코드, 사업소, 팀, 담당자, 섹터로 검색..."
            value={searchTerm}
      ...
      <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl overflow-hidden">
        <div className="px-6 py-4 border-b border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-800/50">
          <h4 className="text-sm font-bold text-zinc-900 dark:text-zinc-100">B2B 거래처별 판매 현황</h4>
          ...
          <table className="w-full text-sm">
            <thead className="bg-zinc-50 dark:bg-zinc-800/50 sticky top-0">
              <tr>
                ...
                <th className="text-left py-3 px-4 text-xs font-bold text-zinc-500 uppercase tracking-wider">B2B팀</th>
                <th className="text-left py-3 px-4 text-xs font-bold text-zinc-500 uppercase tracking-wider">담당자</th>
                <th className="text-left py-3 px-4 text-xs font-bold text-zinc-500 uppercase tracking-wider">섹터분류</th>
      ...
      <div className="text-xs text-zinc-500 dark:text-zinc-400 bg-zinc-50 dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-800 rounded-lg p-4">
        ...
        <ul className="list-disc list-inside space-y-0.5">
          ...
          <li>그룹화: 사업소 → B2B팀 → 담당자 → 섹터분류 → 판매처명</li>
          <li>중량 단위: 리터(L)</li>
          <li>기간: {lastYear}년 vs {currentYear}년</li>
          <li>정렬: 사업소 → B2B팀 → 담당자 → 섹터분류 순</li>
        </ul>
      </div>
    </div>
  );
