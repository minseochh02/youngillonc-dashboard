...
async function main() {
  try {
    ...
    const headers = metadataRows.rows.filter(r => r.일자.includes('회사명') || r.일자.includes('~') || r.일자.length > 10);
    const wrongFormat = metadataRows.rows.filter(r => r.일자.match(/^\d{4}\/\d{2}\/\d{2}$/));
    const timestamps = metadataRows.rows.filter(r => r.일자.match(/\d{4}\/\d{2}\/\d{2}\s+/));
    ...
    `;
    const nullDateRows = await executeSQL(nullDateQuery);
